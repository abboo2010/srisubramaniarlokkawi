...
These sub-tabs are generated live from whatever poojas already exist in Supabase — nothing to configure, and a brand-new recurring pooja type gets its own tab automatically the moment it's added in the Schedule tab, no code change needed. Since each month's occurrence is entered with its own name (e.g. "Monthly Bairavar Pooja (April 2026)", "Monthly Bairavar Pooja (May 2026)", ...), the site automatically strips the trailing "(Month Year)" and the "Monthly"/"Special"/"Pooja"/"Prayer" words to find the pooja's real recurring identity — "Bairavar" — so every month's Bairavar pooja lands on the same tab instead of getting its own. This works with the pooja name in any order (e.g. "Bairavar Pooja (April 2026)" or "Special Bairavar Pooja" both resolve to "Bairavar"), so no particular naming convention needs to be followed when adding new poojas in the Schedule tab.
