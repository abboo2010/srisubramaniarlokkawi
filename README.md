...
Under **Monthly** and **Special**, a second row of tabs appears automatically to split poojas by name (e.g. Bairavar / Shasthi / Pournami) whenever a category has more than one distinct pooja name in it — since those repeat every month, this keeps the schedule from turning into one long mixed pile of same-named cards. Tapping **Monthly Prayers/Poojas** shows these name-tabs with one of them already selected (the first alphabetically) and its poojas already showing below — tapping a different name (e.g. Bairavar) narrows the grid down to just that pooja's own occurrences, same as tapping Annual/Monthly/Special itself narrows to a whole schedule. Same behavior under **Special**. These sub-tabs are generated live from whatever pooja names already exist in Supabase — nothing to configure, and a brand-new recurring pooja type gets its own tab automatically the moment it's added in the Schedule tab, no code change needed. Keep a recurring pooja's name spelled exactly the same every time it's added (e.g. always "Shasthi", not "Shasthi Pooja" one month and "Sashti" the next) so its occurrences group under one tab instead of splitting into two. Annual keeps its original flat list, no sub-tabs (its events are mostly one-off and unique by name, so splitting wouldn't help there).
